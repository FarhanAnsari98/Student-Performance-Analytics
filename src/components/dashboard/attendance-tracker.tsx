...
export function AttendanceTracker() {
  const { user } = useAuth();
  const { teachers, classes, students, attendance, saveAttendanceForClass } =
    useData();
  const { toast } = useToast();

  const teacher = teachers.find(
    (t) => t.id === user?.id.replace("user-", "")
  );
  ...
  const [studentAttendance, setStudentAttendance] =
    useState<StudentAttendanceState>({});
  ...
  useEffect(() => {
    ...
    const attendanceForDay = attendance.filter(
      (a) => a.classId === selectedClassId && a.date === dateStr
    );

    const newState: StudentAttendanceState = {};

    studentsInClass.forEach((student) => {
      const record = attendanceForDay.find(
  ...
  const handleSave = () => {
    ...
    const recordsToSave = Object.entries(studentAttendance).map(
      ([studentId, status]) => ({
        studentId,
        status,
      })
    ...
    toast({
      ...
      description: `Attendance saved for ${format(
        selectedDate,
        "PPP"
      )}`,
    });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select a class and a date to mark attendance.
        </CardDescription>
      </CardHeader>
